...
import { ArrowUpRight, Building2, GraduationCap, Users, ChevronRight } from 'lucide-react';
import { Link } from 'wouter';
import Header from '@/components/Header';
import Footer from '@/components/Footer';
import RegionalMap, { hubs, statusConfig } from '@/components/RegionalMap';
    ...
    <div className="min-h-screen bg-[--paper] text-[--ink]">
      ...
      <section className="relative overflow-hidden border-b border-white/[0.05] pt-[calc(3.5rem+2rem)]">
        <div className="pointer-events-none absolute inset-0" aria-hidden>
          ...
          <div
            ...
            style={{ background: 'radial-gradient(circle, rgba(0,255,133,0.048) 0%, transparent 70%)' }}
          />
        </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {(Object.entries(statusConfig) as any[]).map(([key, sc]: any) => {
              ...
              return (
                <div
                  key={key}
                  className="flex items-center gap-2 border border-white/[0.06] bg-white/[0.02] px-3.5 py-2"
                >
                  <span
                  ...
                  >
                    ...
                    <span className="relative inline-flex h-2 w-2 rounded-full"
                          style={{ background: sc.labelColor, opacity: key === 'expansao' ? 0.45 : 1 }} />
                  </span>
                  <span className="font-mono text-[10px] font-bold uppercase tracking-[0.2em]"
                        style={{ color: key === 'expansao' ? 'rgba(250,250,250,0.28)' : sc.labelColor + 'aa' }}>
                    {sc.label}
                  </span>
                  <span className="font-mono text-[10px] text-[--ink]/25">({count})</span>
                </div>
              );
            })}
          </div>
            ...
            {allOperational.map((hub, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="group flex flex-col gap-5 bg-[--paper] p-7 transition-all duration-300 hover:bg-white/[0.018]"
                >
                  {/* Header do card */}
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      ...
                      style={{ color: sc.labelColor + (hub.status === 'planejamento' ? '80' : 'cc') }}
                    >
                      {hub.status === 'abertura' && (
                  ...
                  <div>
                    ...
                    <h3 className="mt-1 font-display text-2xl font-bold text-[--ink]/82">{hub.name}</h3>
                    <p className="mt-0.5 text-[11.5px] font-semibold uppercase tracking-[0.1em] text-[--ink]/32">
                      {hub.specialty}
                    </p>
                  </div>

                  <p className="flex-1 text-[12.5px] leading-[1.85] text-[--ink]/35">{hub.description}</p>

                  {/* Barra inferior */}
                  <div
                    className="h-px w-full transition-all duration-500 group-hover:opacity-80"
                    style={{ background: `linear-gradient(90deg, ${hub.color}40 0%, transparent 100%)` }}
                  />
                </motion.div>
              );
            })}
      ...
      <section className="border-b border-white/[0.05]">
        <div className="mx-auto max-w-[1280px] px-6 py-14 sm:px-8 lg:px-14">
          ...
          >
            Próximos polos
            <span className="font-light italic" style={{ color: '#FF6B3560' }}> em formação</span>.
          </h2>
            ...
            {expansaoHubs.map((hub, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <p className="font-mono text-[9.5px] font-bold uppercase tracking-[0.2em] text-[--ember]/40">{hub.tag}</p>
                    <h3 className="mt-0.5 font-display text-xl font-bold text-[--ink]/45">{hub.name}</h3>
                    <p className="mt-0.5 text-[11.5px] text-[--ink]/25">{hub.specialty}</p>
                    <p className="mt-2.5 text-[12.5px] leading-relaxed text-[--ink]/30">{hub.description}</p>
                  </div>
                </motion.div>
              );
            })}
      ...
      <section>
        <div className="mx-auto grid max-w-[1280px] border-t border-white/[0.04] sm:grid-cols-3">
          {[
            {
              icon: Building2,
            ...
            <Link
              key={href}
              href={href}
              className="group flex cursor-pointer items-start justify-between border-b border-r border-white/[0.04] p-8 transition-all duration-200 last:border-r-0 hover:bg-white/[0.018] sm:border-b-0"
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-0.5 max-w-[180px] text-[11.5px] text-[--ink]/28">{sub}</p>
                </div>
              </div>
