/**
 * AuthPortalLayout — Compound Component System
 *
 * Composition pattern:
 *   <AuthPortalLayout.Root tone="green" personaKey="empresa">
 *     <AuthPortalLayout.Sidebar>
 *       <AuthPortalLayout.Nav />
 *       <AuthPortalLayout.Hero eyebrow="..." title="..." description="..." />
 *       <AuthPortalLayout.Badges items={metadata} />
 *       <AuthPortalLayout.Stats metrics={metrics} highlights={highlights} icon={Icon} />
 *     </AuthPortalLayout.Sidebar>
 *     <AuthPortalLayout.Main portalLabel="..." icon={Icon}>
 *       {children}
...
interface AuthPortalCtx {
  primary: (typeof toneStyles)[AccentTone];
  secondary: (typeof toneStyles)[AccentTone] | null;
  personaKey?: string;
}

const AuthPortalContext = createContext<AuthPortalCtx | null>(null);

function useAuthPortal() {
  const ctx = useContext(AuthPortalContext);
  if (!ctx) throw new Error("AuthPortalLayout sub-component used outside Root");
  return ctx;
}
...
function Root({ tone, secondaryTone, personaKey, children }: RootProps) {
  const primary = toneStyles[tone];
  const secondary = secondaryTone ? toneStyles[secondaryTone] : null;

  return (
    <AuthPortalContext.Provider value={{ primary, secondary, personaKey }}>
...
function Sidebar({ icon: PortalIcon, children }: SidebarProps) {
  const { primary } = useAuthPortal();
...
function Hero({ icon: PortalIcon, portalLabel, eyebrow, title, description }: HeroProps) {
  const { primary } = useAuthPortal();
...
function Badges({ items }: BadgesProps) {
...
function Stats({ metrics = [], highlights = [] }: StatsProps) {
  const { primary } = useAuthPortal();
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {metrics.map((metric) => (
            ...
            >
              ...
              />
              {metric.context && (
                <p className="mt-3 text-sm leading-6 text-white/52">{metric.context}</p>
              )}
            </div>
          ))}
        </div>
...
function Main({ portalLabel, icon: PortalIcon, children }: MainProps) {
  const { primary, secondary, personaKey } = useAuthPortal();
  const reduceMotion = useReducedMotion();
  ...
  return (
    <main className="relative flex items-center justify-center px-4 py-7 sm:px-6 sm:py-10 lg:px-8 xl:px-10">
      <motion.div
        ...
        // View Transition: shared element morph from LoginHub persona card
        style={
          personaKey
            ? ({ viewTransitionName: `persona-card-${personaKey}` } as CSSProperties)
            : undefined
        }
        className={cn(
